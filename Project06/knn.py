...
class KNN:
    ...
    def predict(self, data, k):
        ...
        for d in range(data.shape[0]):
            distance = self.exemplars - data[d,:]
            distance = np.square(distance)
            distance = np.sum(distance,axis=1)
            distance = np.sqrt(distance)
            
            #sort classes given distcance and keep n values
            sortInd = np.argsort(distance)
            labels = self.classes[sortInd[:k]]
        
            #count the amount of times a value appears
            predi = np.bincount(labels.astype('int'))
            pred.append(np.argmax(predi))
    ...
    def accuracy(self, y, y_pred):
        ...
        '''
        acc = np.count_nonzero(np.where(y == y_pred)[0]) / y.shape[0]
        return acc
    ...
    def plot_predictions(self, k, n_sample_pts):
        ...
        spaced_array = np.linspace(-40,40, n_sample_pts)

        x,y = np.meshgrid(spaced_array, spaced_array)
        xy = np.column_stack((x.flatten(), y.flatten()))
        y_pred = self.predict(xy, k).reshape((n_sample_pts, n_sample_pts))

        fig,ax = plt.subplots(1,1, figsize = (10,5))

        ax.pcolormesh(x,y,y_pred, cmap ='PuOr')
        fig.colorbar(ax.pcolormesh(x,y,y_pred, cmap = 'PuOr'))
